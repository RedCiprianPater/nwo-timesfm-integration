...
def predict_maintenance():
    ...
    if not data or 'history' not in data:
        return jsonify({"error": "Invalid request", "message": "history array required", "code": "INVALID_INPUT"}), 400
    history = data.get('history', [])
    if len(history) < 10:
        return jsonify({"error": "Invalid request", "message": "history must contain at least 10 data points", "code": "INVALID_INPUT"}), 400
    robot_id = data.get('robot_id', 'unknown')
    metric = data.get('metric', 'generic')
    horizon = min(data.get('horizon', 24), 128)
    forecast_data = predictor.predict(history, horizon)
    forecast_values = forecast_data['forecast']
    threshold = min(history) * 0.2
    ...
    return jsonify({
        ...
        "confidence_intervals": {"p10": forecast_data['p10'], "p50": forecast_data['p50'], "p90": forecast_data['p90']},
...
def predict_task_duration():
    ...
    if not data or 'history_durations' not in data:
        return jsonify({"error": "Invalid request", "message": "history_durations array required", "code": "INVALID_INPUT"}), 400
    history = data.get('history_durations', [])
    if len(history) < 5:
        return jsonify({"error": "Invalid request", "message": "history_durations must contain at least 5 data points", "code": "INVALID_INPUT"}), 400
...
def detect_anomaly():
    ...
    if not data or 'readings' not in data:
        return jsonify({"error": "Invalid request", "message": "readings array required", "code": "INVALID_INPUT"}), 400
    readings = data.get('readings', [])
    if len(readings) < 10:
        return jsonify({"error": "Invalid request", "message": "readings must contain at least 10 data points", "code": "INVALID_INPUT"}), 400
    ...
    if anomaly_probability > 0.9:
        severity = "critical"; action = "immediate_maintenance"
    elif anomaly_probability > 0.7:
        severity = "high"; action = "inspect_sensor"
    elif anomaly_probability > 0.5:
        severity = "medium"; action = "monitor_closely"
    else:
...
def predict_swarm_load():
    ...
    if not data or 'task_queue_history' not in data:
        return jsonify({"error": "Invalid request", "message": "task_queue_history array required", "code": "INVALID_INPUT"}), 400
    history = data.get('task_queue_history', [])
    if len(history) < 5:
        return jsonify({"error": "Invalid request", "message": "task_queue_history must contain at least 5 data points", "code": "INVALID_INPUT"}), 400
...
def ratelimit_handler(e):
    return jsonify({"error": "Rate limit exceeded", "message": str(e.description), "retry_after": 60}), 429
